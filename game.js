...
function opsy(text,speak=true){
  ...
  if(speak&&SOUND&&'speechSynthesis' in window){
    try{speechSynthesis.cancel();
      const u=new SpeechSynthesisUtterance(text);u.rate=1.12;u.pitch=1.6;u.volume=.9;
...
const DRONES=[
 {id:'M3E', name:'Mavic 3E', sprite:'drone_m3e', speed:30, drainBase:100/110, thermal:false},
 {id:'M2T', name:'Mavic 2T', sprite:'drone_m2t', speed:27, drainBase:100/85, thermal:true},
 {id:'MINI',name:'Mini 4 Pro',sprite:'drone_mini',speed:36, drainBase:100/70, thermal:false},
];
let drones=[];
...
function corridorFor(m){
  // requests in the north-east fly the dirt-track corridor, not the herd diagonal
  if(m.target.x>=22*TS&&m.target.y<=8*TS) return [{x:11*TS,y:7*TS},{x:30*TS,y:6.25*TS}];
  return [];
}
function legsFor(m){
...
function gateCheck(d,m){
  ...
  if(need>d.batt-20) fails.push({rule:'ENERGY RESERVE', text:'Needs '+need.toFixed(0)+'% of battery'+(windMult>1?' in this headwind':'')+'; aircraft holds '+d.batt.toFixed(0)+'% and the floor is 20% (Seewald et al. 2022).', card:'energy'});
  ...
  for(const h of herds){
    ...
    if(legsFor(m).some(([p,q])=>segCircle(p.x,p.y,q.x,q.y,h.x,h.y,h.buffer+8)))
      fails.push({rule:'WILDLIFE BUFFER', text:'The route cuts the '+h.kind+' herd’s noise buffer'+(h.mixed?' (mixed-species: larger by evidence)':'')+' (Afridi et al. 2026).', card:'noise'});
  }
  // 3. thermal requirement
  if(m.needsThermal&&!d.thermal) fails.push({rule:'PAYLOAD', text:'Ranger support needs the thermal camera. Assign the Mavic 2T.', card:null});
  // 4. controlled airspace
  if(crossesSector(m)&&clearance.state!=='granted')
    fails.push({rule:'CONTROLLED AIRSPACE', text:'The target sits in the controlled sector. Request clearance from the tower first; they answer on their own clock.', card:null, clearance:true});
  // 5. deconfliction
  for(const op of activeOps){
    if(legsFor(m).some(([p,q])=>segCircle(p.x,p.y,q.x,q.y,op.m.target.x,op.m.target.y,56)))
      fails.push({rule:'AIRSPACE CONFLICT', text:'The corridor intersects active '+op.m.id+'. One volume, one aircraft (U-space planning, 2024).', card:'uspace'});
  }
  return fails;
}
  ...
  for(const m of deck){
    ...
    if(m.state==='queued'){
      inner+='<div class="assign">'+drones.filter(d=>d.active&&d.state==='home'&&!d.op).map(d=>
        '<button data-m="'+m.id+'" data-d="'+d.id+'">'+d.id+' '+Math.round(d.batt)+'%</button>').join('')+
...
function assign(mid,did){
  ...
  if(fails.length){
    refusals++; snd.refuse();
    g.innerHTML='<div class="ref">REFUSED</div>'+fails.map(f=>'<div class="frow"><b>'+f.rule+'</b> '+f.text+'</div>').join('')+
    ...
    opsy('Refused: '+fails[0].rule.toLowerCase()+'. The gate never argues taste, only rules.');
  } else {
...
function updateL2(dt){
  ...
  if(clearance.state==='pending'){clearance.t-=dt;if(clearance.t<=0){clearance={state:'granted',t:0};snd.radio();opsy('Tower grants clearance for the controlled sector. Windows are short; fly it while it is yours.');renderDeck();}}
  // wind event at 12:00
  if(!wind.announced&&SHIFT.t>12*3600){wind.on=true;wind.announced=true;snd.warn();opsy('Wind advisory: stiff northerly until mid-afternoon. Northern legs now cost nearly double the battery; the gate has updated its arithmetic.');renderDeck();}
...
function botScore(){
  ...
  return {score:Math.round(s+Math.max(0,meanBatt)*5), completed:DECK.length, disturbs:0, lost:0, eff:Math.round(Math.max(0,meanBatt))};
}
...
function endShift(){
  ...
  const you={score:Math.max(0,score+Math.round(meanBatt*5)), completed:completedOps, disturbs, lost, eff:Math.round(meanBatt)};
  const bot=botScore();
...
function updateShared(dt){
  for(const h of herds){
    ...
    else{h.x+=(h.hx-h.x)*0.15*dt+Math.sin(performance.now()/4000+h.y)*0.06;h.y+=(h.hy-h.y)*0.15*dt;}
...
$('#desk-btn').addEventListener('click',startL2);
